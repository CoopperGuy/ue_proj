...
void ADiaSkillBase::OnSkillStart()
{
	...
	if (IsValid(SkillOwner))
	{
		...
		{
			//UE_LOG(LogARPG, Warning, TEXT("몽타주 유효함: %s"), *SkillMontage->GetName());
			// 몽타주를 직접 재생
			...
			if (IsValid(ownerCharacter))
			{
				...
				if (IsValid(animInstance))
				{
					...
					//UE_LOG(LogARPG, Warning, TEXT("몽타주 재생 시작, 길이: %f"), duration);
				}
			}
		}
		else
		{
			ARPG_SKILL_LOG(Error, TEXT("몽타주가 유효하지 않음"));
		}
	}
}
...
void ADiaSkillBase::InitializeWithData(const FSkillData* InSkillData)
{
	...
	{
		ARPG_SKILL_LOG(Error, TEXT("Null SkillData provided to InitializeWithData"));
		return;
	}
	...
	ARPG_SKILL_VLOG(TEXT("스킬 초기화 - 이름: %s"), *SkillData.SkillName.ToString());
	ARPG_SKILL_VLOG(TEXT("스킬 설명: %s"), *SkillData.SkillDescription.ToString());
	ARPG_SKILL_VLOG(TEXT("쿨다운: %.2f"), SkillData.Cooldown);
	ARPG_SKILL_VLOG(TEXT("마나 소모량: %.2f"), SkillData.ManaCost);
	ARPG_SKILL_VLOG(TEXT("데미지: %.2f"), SkillData.Damage);
#endif
}
